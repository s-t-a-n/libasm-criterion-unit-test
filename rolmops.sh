...
_compile()
{
	...
	make -C $LIBASM_FOLDER re && make re
	result=$?
	...
	then
		...
		logp warning "--- Don't forget to REMOVE -fsanitize=address from your flags (like -Wall -Werror -Wextra) in your libasm makefile before compiling! (since you are no running with segfault flag (-s))"
		exit 1;
	fi
...
Options:
        ...
        -r -> use randomized tests [1/0]
              -------------------------------------------------------------------------------
Run commands:
        compile     / c	 -> compile your libasm and compile tests with your libasm.a
        run         / r  -> run tests
...
cat<<EOF
 ...
 \/_|::\/:/  / \:\  \ /:/  / \:\  \    \/__/~~/:/  / \:\  \ /:/  / \/__\:\/:/  / \:\ \:\ \/__/ 
    ...
    |:|  |        \::/  /       \:\__\     /:/  /       \::/  /                     \::/  /    
     \|__|         \/__/         \/__/     \/__/         \/__/                       \/__/     
                                         A LIBASM UNIT TEST - MAY YOUR SEGFAULTS REST IN PEACE
EOF"
logp beginsection
}
